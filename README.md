...
We can do the same for sets via `assert-set-like`, for queues via `assert-queue-like`.  For maps, `assert-map-like` takes two generators, one for keys, and another for values.
